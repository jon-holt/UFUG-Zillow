...
Load the non-spatial data. Note that numeric variables have been log-transformed. For data description (including data collection, filtering, and merging), please see (link).
...
Plot the ZHVI for Mecklenburg County. This is the median price per square foot. 
...
It is important to remember that coefficients of spatial autoregressive models are interpreted differently than linear models. We must consider the direct, indirect, and total effects of each variable (see paper for explanation). 

```{r}
# calculate the direct, indirect, and total effects
summary(impacts(mod, listw = W, R=2000), zstats=TRUE, short=TRUE)
```
